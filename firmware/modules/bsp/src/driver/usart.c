...
void init_debug_usart() {
    ...
    GPIOA->CRL &= ~(GPIO_CRL_CNF2 | GPIO_CRL_CNF3 | GPIO_CRL_MODE2 |
                    GPIO_CRL_MODE3); // reset CRL reg.
    ...
    USART2->BRR |= (4 << USART_BRR_DIV_Mantissa_Pos) |
                   (5 << USART_BRR_DIV_Fraction_Pos); // 115200
    // USART2->BRR |= (52 << USART_BRR_DIV_Mantissa_Pos) | (1 <<
    // USART_BRR_DIV_Fraction_Pos); // 9600
    ...
    USART2->CR1 |=
        USART_CR1_TE | USART_CR1_RE; // enable transmitter and receiver:
